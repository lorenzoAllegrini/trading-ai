...
class Labeller:
    ...
    def label_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        max_lookback = max(
            rule.lookback for rule in self.strategy.rules
        )  # modificare: massimo anche rispetto al lookback degli indicatori
        ...
        if isinstance(df.index, pd.MultiIndex):
            rename_map[out.columns[0]] = "level_0"
            rename_map[out.columns[1]] = "level_1"
        else:
            rename_map[out.columns[0]] = "level_1"
