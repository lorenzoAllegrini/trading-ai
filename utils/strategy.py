...
class MLStrategy:
    def __init__(
        ...
        if "atr" not in indicator_factories:
            ...
            indicator_factories["atr"] = lambda: IndicatorWrapper(
                AverageTrueRange(atr_period, MovingAverageType.Wilders), "atr", symbol
            )
    ...
    def preprocess(self, history_window: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        rename_map = {}
        if isinstance(history_window.index, pd.MultiIndex):
            rename_map[price_data_all.columns[0]] = "level_0"
            rename_map[price_data_all.columns[1]] = "level_1"
        else:
            rename_map[price_data_all.columns[0]] = "level_1"

        price_data_all.rename(columns=rename_map, inplace=True)
        ...
        combined_out = combined_out.drop(columns=["level_0", "level_1"], errors="ignore")
